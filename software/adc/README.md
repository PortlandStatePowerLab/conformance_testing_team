...
- `adc_aquisition_diagnostic.py`: private, reusable grouped-versus-single acquisition diagnostic logic.
...
- Pysical-station wiring choices, such as the installed I2C bus, device address, and
...
- An interface describes the minimum ADC read operations that protable sensor code may request.
...
- A device builder constructs and configures that driver using connection detials supplied by its caller.
...
Portable sensor code should recieve a `SensorAdc` object from its caller istead of constructing hardware.
...
Calling `build_max1238()` opens the selected I2C bus, constructs the physcial ADC driver,
...
ADC object before allowing the failure to propogate.
